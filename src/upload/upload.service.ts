...
export class UploadService {
  ...
  async upload(bodys: any): Promise<any> {
    ...
    try {
      ...
      if (!!findOne) {
        throw new HttpException({ error: '当前文件已存在' }, 502)
      }
      // 存储二进制文件
      ...
      if (!saveFile) {
        throw new HttpException({ error: '存储文件失败' }, 502)
      }
      ...
      if (Number(mergeRedis) === Number(chunkAll)) {
        ...
        if (!mergeFile) {
          throw new HttpException({ error: '合并文件失败' }, 502)
        }
        // 存储数据库
        const obj = {
          type: 2,
        ...
        if (!save) {
          throw new HttpException({ error: '存储数据库失败' }, 502)
        }
        // 删除临时数据
        for (let index = 0; index < Number(mergeRedis); index++){
